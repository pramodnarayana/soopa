...
export function FilterBuilder({ availableFields, filters, onChange }: FilterBuilderProps) {
  ...
  const updateFilter = (index: number, key: keyof FilterRule, val: any) => {
    const newFilters = [...filters]
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-end">
        <Button size="sm" variant="outline" onClick={addFilter} className="h-8 gap-1.5 text-xs bg-white">
      ...
      {filters.length > 0 && (
        <div className="space-y-2">
          {filters.map((f, i) => (
            <div key={f.id || i} className="flex items-center justify-end gap-2 bg-white p-2 rounded-lg border border-slate-200">
              <select
              ...
              <Button size="icon" variant="ghost" className="h-9 w-9 text-slate-400 hover:text-red-500 hover:bg-red-50" onClick={() => removeFilter(i)}>
                <X className="w-4 h-4" />
              </Button>
            </div>
          ))}
