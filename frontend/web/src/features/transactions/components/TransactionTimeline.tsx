...
import { FileJson, Server, Activity, CheckCircle2, AlertCircle, Database } from 'lucide-react'
...
export function TransactionTimeline({ transaction }: Props) {
  ...
  const renderBadge = (status?: string) => {
    ...
    if (['RECEIVED', 'ACCEPTED', 'PARSED', 'TRANSFORMED', 'DELIVERED'].includes(upper)) {
      return <Badge variant="outline" className="bg-emerald-50 text-emerald-700 border-emerald-200">SUCCESS ({upper})</Badge>
    }
    if (['FAILED', 'ERROR'].includes(upper)) {
      return <Badge variant="outline" className="bg-red-50 text-red-700 border-red-200">FAILURE ({upper})</Badge>
    }
    return <Badge variant="outline" className="bg-slate-50 text-slate-700 border-slate-200">{upper}</Badge>
  }
        ...
        {jsons.length === 0 ? (
          ...
          jsons.map((json, idx) => {
            const reconstructedPayload = {
              trading_partner_id: json.business_metadata?._routing?.trading_partner_id || 'UNKNOWN',
            ...
            return (
              <div key={json.id} className={idx > 0 ? 'pt-6 border-t border-slate-100' : ''}>
                <div className="text-xs font-semibold text-slate-500 uppercase mb-2">Request Payload</div>
                <CodeViewer
                  language="json"
                  height={250}
          ...
          jsons.map((json, idx) => (
            <div key={json.id} className={idx > 0 ? 'pt-6 border-t border-slate-100' : ''}>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  <div className="text-xs font-semibold text-slate-500 uppercase mb-2">Business Metadata</div>
                  <CodeViewer
                    language="json"
                    height={250}
                ...
                <div>
                  <div className="text-xs font-semibold text-slate-500 uppercase mb-2">JSON Payload</div>
                  <CodeViewer
                    language="json"
                    height={250}
                    value={json.payload ? JSON.stringify(json.payload, null, 2) : 'No payload available.'}
                  />
                </div>
              </div>
            </div>
    ...
    <Card>
      ...
      <CardContent className="pt-4 space-y-6">
        {gateways.length === 0 ? (
          ...
          gateways.map((gw, idx) => {
            const isSuccess = gw.http_status_code && gw.http_status_code >= 200 && gw.http_status_code < 300
            return (
              <div key={gw.id} className={idx > 0 ? 'pt-6 border-t border-slate-100' : ''}>
                <div className="flex items-center justify-between mb-4 bg-slate-50 p-3 rounded-lg border border-slate-100">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <div className="font-medium text-slate-900">{gw.webhook_url ? `Webhook: ${gw.webhook_url}` : 'Webhook'}</div>
                      <div className="text-xs text-slate-500">{new Date(gw.created_at).toLocaleString()}</div>
                    </div>
                  </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div>
                    ...
                    <CodeViewer
                      language="json"
                      height={200}
                      value={gw.payload ? JSON.stringify(gw.payload, null, 2) : 'No payload available'}
                    />
                  </div>
                  <div>
      ...
      <div className="relative pl-6 border-l-2 border-slate-100 space-y-12 pb-8">
        ...
        <div className="relative">
          <div className="absolute -left-[35px] bg-white p-1 rounded-full border-2 border-indigo-500">
            {isOutbound ? <FileJson className="w-5 h-5 text-indigo-600" /> : <Server className="w-5 h-5 text-indigo-600" />}
          </div>
        ...
        <div className="relative">
          <div className="absolute -left-[35px] bg-white p-1 rounded-full border-2 border-blue-500">
            {isOutbound ? <Database className="w-5 h-5 text-blue-600" /> : <FileJson className="w-5 h-5 text-blue-600" />}
          </div>
          {renderEdiJsonBlock()}
        </div>
