...
from identity_jobs_worker.adapters.inbound.jobs.identity_outbox_cleanup_job import (
    IdentityOutboxCleanupJobHandler,
)
from identity_jobs_worker.adapters.inbound.jobs.identity_outbox_sweeper_job import (
    IdentityOutboxSweeperJobHandler,
)
from identity_jobs_worker.config.settings import AppSettings

logger = structlog.get_logger(__name__)


class WorkerContainer:
    """Dependency Injection container for the Identity Jobs Worker."""
    ...
    def _wire_jobs_consumer(self) -> None:
        ...
        job_dispatcher.subscribe(
            IdentityJobName.IDENTITY_OUTBOX_CLEANUP.value, outbox_cleanup_handler
        )
