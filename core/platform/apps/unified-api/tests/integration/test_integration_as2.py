...
@pytest.mark.asyncio
async def test_global_as2_partners_crud(auth_client: httpx.AsyncClient, seeded_api_token: dict):
    """
    Test the AS2 Partners API on the global level.
    """
    # 1. List AS2 Partners
    res = await auth_client.get("/api/v1/as2/trading-partners")
    assert res.status_code == 200

    # 2. Create a local AS2 partner
    res = await auth_client.post(
        "/api/v1/as2/trading-partners",
        json={
    ...
    res = await auth_client.put(
        f"/api/v1/as2/trading-partners/{remote_partner_id}",
        json={
    ...
    res = await auth_client.delete(f"/api/v1/as2/trading-partners/{remote_partner_id}")
    assert res.status_code == 204

    # 6. Delete the local partner
    res = await auth_client.delete(
        f"/api/v1/as2/trading-partners/{partner_id}",
    )
    assert res.status_code == 204
