...
async def test_as2_partnership_connection(  # noqa: C901
    partnership_id: str,
...
async def update_platform_as2_partnership(
    ...
    try:
        async with uow:
            mdn_url_val = (
                str(request.mdn_url)
                if request.mdn_url is not None
                else None
                if "mdn_url" in request.model_fields_set
                else UNSET
            )
            mdn_type_val = (
                MDNType(request.mdn_type)
                if request.mdn_type is not None
                else None
                if "mdn_type" in request.model_fields_set
                else UNSET
            )
            enc_alg_val = (
                EncryptionAlgorithm(request.encryption_algorithm)
                if request.encryption_algorithm is not None
                else None
                if "encryption_algorithm" in request.model_fields_set
                else UNSET
            )
            sig_alg_val = (
                SignatureAlgorithm(request.signature_algorithm)
                if request.signature_algorithm is not None
                else None
                if "signature_algorithm" in request.model_fields_set
                else UNSET
            )

            cmd = UpdateAS2PartnershipCmd(
                name=request.name if "name" in request.model_fields_set else UNSET,
                credentials_vault_ref=request.credentials_vault_ref
                if "credentials_vault_ref" in request.model_fields_set
                else UNSET,
                mdn_type=mdn_type_val,
                mdn_url=mdn_url_val,
                encryption_algorithm=enc_alg_val,
                signature_algorithm=sig_alg_val,
                advanced_flags=request.advanced_flags
                if "advanced_flags" in request.model_fields_set
                else UNSET,
                active=request.active if "active" in request.model_fields_set else UNSET,
            )
