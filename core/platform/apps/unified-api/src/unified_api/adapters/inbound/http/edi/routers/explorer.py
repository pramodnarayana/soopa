...
router = APIRouter(prefix="/api/v1/tenants/{tenant_id}/edi/explorer", tags=["Explorer"])
...
def _serialize_edi_message(msg: EdiMessageDTO) -> JsonDict:
    return {
        "id": str(msg.id),
...
def _serialize_edi_json(j: EdiJsonDTO) -> JsonDict:
    return {
        "id": str(j.id),
