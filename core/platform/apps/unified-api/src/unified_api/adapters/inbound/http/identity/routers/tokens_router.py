...
from unified_api.adapters.inbound.http.identity.dtos.api_token_dtos import (
...
async def list_tokens(
    ...
    use_case_factory: Callable[..., ListApiTokensUseCase] = Depends(
        Provide[Container.list_api_tokens_use_case.provider]
    ),
...
async def create_token(
    ...
    context: Annotated[IdentityContext, Depends(require_tenant_member)],
    session: AsyncSession = Depends(get_db_session),
    use_case_factory: Callable[..., CreateApiTokenUseCase] = Depends(
        Provide[Container.create_api_token_use_case.provider]
    ),
) -> ApiTokenCreatedResponse:
    """
...
async def update_token(
    ...
    context: Annotated[IdentityContext, Depends(require_tenant_member)],
    session: AsyncSession = Depends(get_db_session),
    use_case_factory: Callable[..., UpdateApiTokenUseCase] = Depends(
        Provide[Container.update_api_token_use_case.provider]
    ),
) -> ApiTokenResponse:
...
async def delete_token(
    ...
    context: Annotated[IdentityContext, Depends(require_tenant_member)],
    session: AsyncSession = Depends(get_db_session),
    use_case_factory: Callable[..., DeleteApiTokenUseCase] = Depends(
        Provide[Container.delete_api_token_use_case.provider]
    ),
) -> None:
