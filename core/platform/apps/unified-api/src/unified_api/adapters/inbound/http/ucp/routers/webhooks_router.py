...
from unified_api.adapters.inbound.http.ucp.dtos.webhook_dtos import (
...
def _webhook_response(webhook: WebhookDomainModel, tenant_id: str) -> WebhookResponse:
...
async def list_webhooks(
    ...
    use_case_factory: Callable[..., ListWebhooksUseCase] = Depends(
        Provide[Container.list_webhooks_use_case.provider]
    ),
...
async def create_webhook(
    ...
    context: Annotated[IdentityContext, Depends(require_tenant_member)],
    session: AsyncSession = Depends(get_db_session),
    use_case_factory: Callable[..., CreateWebhookUseCase] = Depends(
        Provide[Container.create_webhook_use_case.provider]
    ),
...
async def update_webhook(
    ...
    context: Annotated[IdentityContext, Depends(require_tenant_member)],
    session: AsyncSession = Depends(get_db_session),
    use_case_factory: Callable[..., UpdateWebhookUseCase] = Depends(
        Provide[Container.update_webhook_use_case.provider]
    ),
    idempotency_key: str | None = Header(None, alias="Idempotency-Key"),
) -> WebhookResponse:
...
async def delete_webhook(
    ...
    context: Annotated[IdentityContext, Depends(require_tenant_member)],
    session: AsyncSession = Depends(get_db_session),
    use_case_factory: Callable[..., DeleteWebhookUseCase] = Depends(
        Provide[Container.delete_webhook_use_case.provider]
    ),
    idempotency_key: str | None = Header(None, alias="Idempotency-Key"),
) -> None:
