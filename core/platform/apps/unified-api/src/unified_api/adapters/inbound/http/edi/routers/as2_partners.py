...
from unified_api.adapters.inbound.http.edi.dtos.dtos import CertificateExportResponse
...
async def _rotate_as2_certificates(
    ...
    tenant_id: str,
    uow: ControlPlaneUnitOfWorkPort,
...
async def delete_certificate_secret(
    ...
    parts = vault_ref.replace("vault://", "").split("/")
    if (
        not vault_ref.startswith("vault://")
        or len(parts) < 1
        or parts[0] != SecretCategory.AS2_KEY.value
    ):
