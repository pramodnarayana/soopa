...
async def repo(db_session_factory):
    return PostgresUserNotificationPreferenceRepository(session_factory=db_session_factory)


@pytest.mark.asyncio
async def test_save_and_get_preference(
    repo: PostgresUserNotificationPreferenceRepository, db_session_factory
):
    # Arrange
