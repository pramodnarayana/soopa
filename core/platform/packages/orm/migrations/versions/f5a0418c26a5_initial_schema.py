...
def upgrade() -> None:
    ...
    # ---------------------------------------------------------------------------
    # Custom Postgres Triggers for Notification Outbox
    op.execute("""
        CREATE OR REPLACE FUNCTION notifications_notification_outbox_notify() RETURNS trigger AS $$
        BEGIN
            PERFORM pg_notify('notification_outbox_channel', NEW.id::text);
            RETURN NEW;
        END;
        $$ LANGUAGE plpgsql;
    """)
    op.execute("""
        CREATE TRIGGER trg_notifications_notification_outbox_notify
        AFTER INSERT ON notifications.notification_outbox
        FOR EACH ROW EXECUTE FUNCTION notifications_notification_outbox_notify();
    """)
    # ---------------------------------------------------------------------------
...
def downgrade() -> None:
    ...
    # ### commands auto generated by Alembic - please adjust! ###
    op.execute(
        "DROP TRIGGER IF EXISTS trg_notifications_notification_outbox_notify ON notifications.notification_outbox;"
    )
