...
async def test_outbox_processor_success():
    ...
    processor = NotificationOutboxProcessorUseCase(repo, dispatcher, worker_id="w1")
...
async def test_sweep_stuck_messages():
    ...
    sweeper = NotificationOutboxSweeperUseCase(repo)
    ...
    assert repo.sweep_calls[0] == 30000  # Default lease in NotificationOutboxSweeperUseCase
    assert repo.swept == 5
