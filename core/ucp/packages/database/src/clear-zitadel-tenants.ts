...
const PROTECTED_ORG_IDS = new Set<string>([
  process.env.ZITADEL_PLATFORM_ORG_ID || 'missing_platform_org_id',
]);
...
const APPROVED_HOSTS = ['localhost', '127.0.0.1', 'ucp.localhost', 'zitadel.localhost'];
...
async function clearZitadelTenants() {
  ...
  const isApprovedHost = APPROVED_HOSTS.some(
    (host) => url.hostname === host || url.hostname.endsWith('.' + host),
  );

  if (!isApprovedHost && !force) {
    console.error(
      `ERROR: Safety check failed. API URL ${apiUrl} does not match an approved local/development host.`,
    );
    console.error(`Approved hosts: ${APPROVED_HOSTS.join(', ')}`);
    console.error(
      'To bypass this check, use --force flag or set ZITADEL_CLEANUP_FORCE=true environment variable.',
    );
    process.exit(1);
  }
