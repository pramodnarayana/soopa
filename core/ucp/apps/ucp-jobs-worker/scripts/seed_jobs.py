...
async def main() -> None:
    ...
    jobs = [
        ...
        {
            "name": UcpCleanupJobName.UCP_IDEMPOTENCY_CLEANUP.value,
            "payload": {},
            "status": "PENDING",
            "cron_expression": "* * * * *",
            "target_queue": "ucp-jobs.fifo",
        ...
        {
            "name": UcpCleanupJobName.UCP_AUDIT_LOG_CLEANUP.value,
            "payload": {},
            "status": "PENDING",
            "cron_expression": "* * * * *",
            "target_queue": "ucp-jobs.fifo",
    ...
    try:
        async with engine.begin() as conn:
            logger.info("Seeding UCP scheduler jobs...")
            ...
            logger.info("All UCP scheduler jobs seeded successfully.")
    except Exception:
        logger.exception("Failed to seed UCP scheduler jobs.")
        sys.exit(1)
    finally:
