...
class TenantRepository(ITenantRepository):
    ...
    async def delete(self, tenant_id: str, idempotency_key: Optional[str] = None) -> None:
        ...
        await self.session.execute(delete(ShardRegistry).where(ShardRegistry.tenant_id == tenant_id))
        await self.session.execute(delete(AppSubscription).where(AppSubscription.tenant_id == tenant_id))
        await self.session.execute(delete(ControlPlaneOutbox).where(ControlPlaneOutbox.tenant_id == tenant_id))
