...
import { useGetTenantUsers } from '@/domains/users/api/queries';
import { useCreateTenantUser, useDeleteTenantUser, useToggleTenantUserStatus } from '@/domains/users/api/mutations';
import { useGetRoles } from '@/domains/roles/api/queries';
import { UserPlus, Users, Trash2, Send, X, ChevronDown, ChevronRight } from 'lucide-react';
import { Button } from '@/components/ui/button';
import { Input } from '@/components/ui/input';
import { Select, SelectContent, SelectItem, SelectTrigger, SelectValue } from '@/components/ui/select';
import { createColumnHelper, getCoreRowModel, useReactTable, getExpandedRowModel } from '@tanstack/react-table';
  ...
  const handleCreateUser = () => {
    createMutationObj.mutate(
      { tenantId, firstName, lastName, email, role: selectedRole },
      {
        ...
        onError: (error: any) => toast.error(`Error creating user: ${error.message}`)
      }
    );
  };
    ...
    if (confirm(`Delete user ${email}? This cannot be undone.`)) {
      deleteMutationObj.mutate(
        { tenantId, userId },
        {
          ...
          onError: (error: any) => toast.error(`Error deleting user: ${error.message}`)
        }
      );
    }
  ...
  const columns = React.useMemo(() => [
      ...
      cell: (info) => {
        ...
        return (
          <div className="flex items-center gap-3">
            <div className={`w-9 h-9 rounded-full flex items-center justify-center text-sm font-semibold ${isActive ? 'bg-indigo-100 text-indigo-700' : 'bg-slate-100 text-slate-400'}`}>
              {initials}
            </div>
            <div>
              {user.displayName && (
              ...
              <p className={`text-xs ${user.displayName ? 'text-slate-500' : 'font-medium text-sm text-slate-900'}`}>{info.getValue()}</p>
            </div>
          </div>
        );
      },
    ...
    columnHelper.accessor('role', {
      ...
      cell: (info) => {
        ...
        const roleData = roles.find((r: any) => r.key === role);
        const label = roleData ? roleData.displayName : role;
        const isTenantAdmin = role === 'TenantAdmin';
        return (
          <span className={`inline-flex items-center rounded-full px-2.5 py-1 text-xs font-medium ring-1 ring-inset ${isTenantAdmin ? 'bg-violet-50 text-violet-700 ring-violet-600/20' : 'bg-sky-50 text-sky-700 ring-sky-600/20'}`}>
            {label}
          </span>
        );
      },
      ...
      cell: (info) => {
        ...
        return (
          <div className="flex items-center gap-2 justify-end pr-2">
            ...
            >
              <span className={`h-1.5 w-1.5 rounded-full transition-colors ${isActive ? 'bg-emerald-500' : 'bg-slate-400'}`} />
            ...
            <Button
              ...
              onClick={(e) => { e.stopPropagation(); toast.info('Email invitations coming soon!'); }}
            >
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="relative w-full max-w-md bg-white rounded-2xl shadow-2xl ring-1 ring-slate-200/60">
            <div className="flex items-center justify-between p-6 border-b border-slate-100">
              ...
              <button onClick={resetModal} className="rounded-lg p-1.5 text-slate-400 hover:text-slate-600 hover:bg-slate-100 transition-colors">
                <X className="w-4 h-4" />
              </button>
            </div>
            ...
            <div className="p-6 space-y-4">
              <div className="grid grid-cols-2 gap-3">
                <div className="space-y-1.5">
                  ...
                  <Input id="user-first-name" placeholder="John" value={firstName} onChange={(e) => setFirstName(e.target.value)} className="rounded-lg" />
                </div>
                <div className="space-y-1.5">
                  <label className="text-sm font-medium text-slate-700">Last Name</label>
                  <Input id="user-last-name" placeholder="Doe" value={lastName} onChange={(e) => setLastName(e.target.value)} className="rounded-lg" />
                </div>
              </div>
              <div className="space-y-1.5">
                <label className="text-sm font-medium text-slate-700">Email Address</label>
                <Input id="user-email" type="email" placeholder="john.doe@company.com" value={email} onChange={(e) => setEmail(e.target.value)} className="rounded-lg" />
              </div>
              <div className="space-y-1.5">
              ...
              <div className="rounded-lg bg-slate-50 border border-slate-200 px-4 py-3">
                <p className="text-xs text-slate-500">A default password will be set. The user can update it after first login.</p>
              </div>
            </div>

            <div className="flex items-center justify-end gap-3 px-6 pb-6">
              <Button variant="outline" onClick={resetModal} className="rounded-lg">Cancel</Button>
              <Button
                id="create-user-submit"
                className="bg-indigo-600 hover:bg-indigo-700 text-white rounded-lg px-5"
                disabled={!firstName || !lastName || !email || !selectedRole || createMutationObj.isPending}
      ...
      <div className="space-y-5">
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-sm text-slate-500 mt-0.5">{users.length} user{users.length !== 1 ? 's' : ''} in this tenant</p>
          </div>
          <Button
            id="create-user-btn"
