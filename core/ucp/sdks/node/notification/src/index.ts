...
export async function notify(event: NotificationEvent): Promise<void> {
  ...
  const storage = identityContextStorage as unknown as AsyncLocalStorage<IdentityContext>;
  ...
  if (!tenantId) {
    throw new Error(
      'notify() must be called within an authenticated Identity context (missing tenantId)',
    );
  }
