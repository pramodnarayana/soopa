...
- **No Boolean Control Coupling**: REJECT code that passes boolean flags (e.g., `raise_exceptions=True` or `fail_fast=True`) into core loops, managers, or business logic to alter fundamental control flow. Demand **Polymorphism** and the **Strategy Pattern** instead.
- **Strict Test/Prod Separation**: REJECT any production code that imports testing tools, defines testing strategies, or contains environment branches (e.g., `if app_env == "test":`) to alter dependency injection or logic.
