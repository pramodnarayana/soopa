...
def service(mock_session, mock_vault, mock_db_router):
    svc = As2ReceiveService(mock_session, mock_vault, mock_db_router)
    svc.uow = AsyncMock()
    return svc
...
async def test_process_inbound_message_bad_request(service):
    ...
    with patch("api.services.as2_receive_service.parse_as2_request") as mock_parse:
...
async def test_process_inbound_message_partnership_not_found(service):
    ...
    with patch("api.services.as2_receive_service.parse_as2_request") as mock_parse:
        ...
        service.uow.control_plane.get_partnership_by_as2_ids.return_value = None
...
async def test_process_inbound_message_success(service):
    ...
    with patch("api.services.as2_receive_service.parse_as2_request") as mock_parse:
        ...
        service.uow.control_plane.get_partnership_by_as2_ids.return_value = (
            mock_partnership,
            mock_local_partner,
            mock_remote_partner,
        )
        ...
        with (
            ...
            patch.object(service, "_save_to_data_plane") as mock_save,
        ):
...
async def test_private_methods_coverage(service):
    ...
    # Test _save_to_data_plane (failure path)
...
async def test_crypto_pipeline_coverage(service):
    ...
    with patch("api.services.as2_receive_service.verify_signature") as mock_verify:
    ...
    with patch("api.services.as2_receive_service.decrypt_payload") as mock_decrypt:
...
async def test_save_to_data_plane_success(service):
    ...
    with patch("api.services.as2_receive_service.SqlAlchemyDataPlaneRepository") as mock_repo_cls:
        mock_repo = AsyncMock()
