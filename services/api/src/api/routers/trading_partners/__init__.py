...
async def list_trading_partners(
    ...
    async with uow:
        ...
        if uow.control_plane:
            ...
            as2_partners_global = await uow.control_plane.list_as2_partners(0)
            as2_partners = list(as2_partners) + list(as2_partners_global)

            sftp_partners = await uow.control_plane.list_sftp_partners(tenant_id)
        ...
        for p in sftp_partners:
            partners.append(
                PartnerResponse(
                    ...
                    port=p.port,
                    username=p.username,
                    inbound_remote_path=p.inbound_remote_path,
                    outbound_remote_path=p.outbound_remote_path,
                )
            )
