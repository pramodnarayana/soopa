# ruff: noqa: S603, S607, G004, TRY401
import json
import os
import subprocess
import sys
...
def wait_for_file(file_path: Path, max_wait_ms: int = 60000) -> None:
    ...
    logger.info(f"Waiting for {file_path} to be generated by Zitadel...")
    while (time.time() - start_time) * 1000 < max_wait_ms:
        if file_path.exists():
            logger.info("File found!")
            return
        time.sleep(1)
    raise TimeoutError(f"Timeout waiting for {file_path} after {max_wait_ms}ms")
...
def _destroy_existing(script_dir: Path, root_dir: Path) -> None:
    ...
    for state_file in ["terraform.tfstate", "terraform.tfstate.backup"]:
        ...
        if path.exists():
            path.unlink()
            logger.info(f"  Cleared: {state_file}")
...
def _run_full_bootstrap(script_dir: Path, root_dir: Path, pat_key_path: Path) -> None:
    ...
    # Revoke the ephemeral PAT — it must not linger on disk
    logger.info("Revoking ephemeral PAT by deleting pat.json...")
    if pat_key_path.exists():
        pat_key_path.unlink()
...
if __name__ == "__main__":
    try:
        ...
        logger.exception(f"\n❌ Bootstrap timed out: {e}")
        sys.exit(1)
    except RuntimeError as e:
        logger.exception(f"\n❌ Bootstrap configuration error: {e}")
        sys.exit(1)
