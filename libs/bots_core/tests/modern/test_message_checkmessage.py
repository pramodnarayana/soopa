...
def test_x12_parse_envelope_counts(tmp_path):
...
def test_x12_wrong_ge01_count(tmp_path):
    """GE01 says 2 transactions but only 1 ST exists → error E17."""
    edi = _x12_850(ge01="2")
    f = tmp_path / "bad_ge.edi"
    f.write_text(edi)
    orig = _patch_data_dir(tmp_path)
    try:
        from bots_core.domain.inmessage import parse_edi_file
...
def test_x12_mismatched_gs_ge_reference(tmp_path):
    """GS06 ≠ GE02 → error E16."""
    edi = _x12_850(gs06="1", ge02="9")
    f = tmp_path / "bad_gsge_ref.edi"
    f.write_text(edi)
    orig = _patch_data_dir(tmp_path)
    try:
        from bots_core.domain.inmessage import parse_edi_file
...
def test_x12_mismatched_st_se_reference(tmp_path):
    """ST02 ≠ SE02 → error E19."""
    edi = _x12_850(st02="0001", se02="9999")
    f = tmp_path / "bad_stse_ref.edi"
    f.write_text(edi)
    orig = _patch_data_dir(tmp_path)
    try:
        from bots_core.domain.inmessage import parse_edi_file
...
def test_x12_invalid_iea_count_non_numeric(tmp_path):
    ...
    f = tmp_path / "bad_iea_nonnumeric.edi"
    f.write_text(edi)
    orig = _patch_data_dir(tmp_path)
    try:
        from bots_core.domain.inmessage import parse_edi_file
...
def test_x12_roundtrip_preserves_all_segments(tmp_path):
