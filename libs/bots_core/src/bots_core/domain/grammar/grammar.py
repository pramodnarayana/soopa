...
class Grammar:
    ...
    def _dorecorddefs(self):
        ...
        try:
            self.recorddefs = self.module.recorddefs
        ...
        for recordid, fields in self.recorddefs.items():
            ...
            if isinstance(self, (xml, json, templatehtml)):
                if len(fields) < 1:
                    raise GrammarError(
                        _(
                            'Grammar "%(grammar)s", in recorddefs, record "%(record)s":'
    ...
    def _dostructure(self):
        ...
        """
        try:
            self.structure = self.module.structure
        except AttributeError as exc:
            _exception = GrammarPartMissing(
        ...
        if len(self.structure) != 1:
            print("DEBUG STRUCTURE FAILED LENGTH:", self.structure)
            raise GrammarPartMissing(
                _(
