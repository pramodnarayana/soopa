...
import { ITemplateRepository, IDeliveryService } from '../ports/index.js';
...
export class DispatchNotificationUseCase {
  ...
  public async execute(event: NotificationEvent): Promise<void> {
    ...
    if (targetTemplates.length === 0) {
      console.warn(`No templates found for event ${event.eventType} on channels ${event.channels.join(',')}`);
      return;
    }
