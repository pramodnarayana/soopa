  ...
  implements IProjectProvider
{
  ...
  async deleteProjectGrant(orgId: string, projectId: string): Promise<void> {
    this.logger.log(
    ...
    try {
      ...
      const parsedSearchData =
        ZitadelProjectGrantsResponseSchema.parse(searchData);
      const grant = parsedSearchData.result?.find(
  ...
  async getUsers(
    ...
    let grants: ZitadelProjectGrant[] = [];
    try {
      ...
      if (grantRes.ok) {
        ...
        grants = parsedGrantData.result || [];
      }
    } catch {
      this.logger.warn(`Failed to fetch grants for org ${orgId}`);
    }
