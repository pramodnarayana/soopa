...
function TenantAppsPage() {
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
      {apps.map((app: any) => {
        const isSubscribed = subscriptions.some((sub: any) => sub.appId === app.id && sub.status === 'active');
        ...
        return (
          ...
          >
            ...
            <div className="flex flex-col gap-4 flex-grow mt-2">
              <div className={`w-12 h-12 rounded-xl flex items-center justify-center ${isSubscribed ? 'bg-emerald-600 text-white' : 'bg-slate-100 text-slate-600'} transition-colors`}>
                <Box className="w-6 h-6" />
              </div>
              
              <div className="space-y-1">
                <h3 className={`text-lg font-semibold ${isSubscribed ? 'text-emerald-950' : 'text-slate-900'}`}>
                  {app.name}
                </h3>
