...
import { AS2PartnershipDetails } from './AS2PartnershipDetails';
import { SharedRowActions } from './SharedRowActions';

function AS2PartnershipRowActions({ as2Partnership }: { as2Partnership: AS2Partnership }) {
  ...
  const handleDelete = (e: React.MouseEvent) => {
    e.stopPropagation();
    if (
      confirm('Are you sure you want to delete this as2Partnership? This action cannot be undone.')
    ) {
      deletePlatform.mutate(as2Partnership.id);
    }
  };

  const handleToggleActive = (e: React.MouseEvent) => {
    e.stopPropagation();
    const newActiveState = as2Partnership.active === false ? true : false;
    const payload = { active: newActiveState };
    updatePlatform.mutate({ id: as2Partnership.id, payload });
  };

  return (
    <SharedRowActions
      isActive={as2Partnership.active !== false}
      isUpdating={isUpdating}
      isDeleting={isDeleting}
      onToggleActive={handleToggleActive}
      onDelete={handleDelete}
      entityName="AS2Partnership"
    />
  );
}
          ...
          if (!name)
            return (
              <div className="flex items-center gap-3">
                ...
                <span className="text-slate-400 text-sm">Unnamed AS2Partnership</span>
              </div>
            );
          return (
            <div className="flex items-center gap-3">
      ...
      columnHelper.display({
        ...
        cell: (info) => (
          <div className="flex justify-end">
            <AS2PartnershipRowActions as2Partnership={info.row.original} />
          </div>
        ),
      }),
    ];
