...
export function InboundRouteForm({ onSuccess }: { onSuccess: () => void }) {
  ...
  const { data: allDestinations, isLoading: isLoadingDestinations } = useTenantDestinations(
    Direction.INBOUND,
  );
  ...
  const destinations = (allDestinations || []).filter((d) => {
    ...
    if (processingMode === ProcessingMode.PASSTHROUGH)
      return d.type === DestinationType.AS2 || d.type === DestinationType.SFTP;
    return true;
  });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      toast({
        title: 'Failed to create inbound route',
    ...
    <form onSubmit={handleSubmit} className="flex flex-col gap-6">
      ...
      <div className="bg-slate-50/50 border border-slate-100 p-4 rounded-xl space-y-4">
        ...
        <div className="grid grid-cols-2 gap-4">
          ...
          <div className="grid gap-2">
            <Label htmlFor="processing_mode" className="text-slate-600 font-medium">
              Processing Mode *
            </Label>
            <Select
      ...
      <div className="bg-indigo-50/30 border border-indigo-100 p-4 rounded-xl space-y-4">
        ...
        <div className="grid grid-cols-4 gap-4">
          ...
          <div className="grid gap-2">
            <Label htmlFor="gs_sender" className="text-slate-600 font-medium text-sm">
              GS Sender ID
            </Label>
            <Input
              id="gs_sender"
              ...
              placeholder="PARTNER_GS"
              className="h-10 bg-white font-mono text-sm uppercase"
            />
          </div>
          <div className="grid gap-2">
            <Label htmlFor="gs_receiver" className="text-slate-600 font-medium text-sm">
              GS Receiver ID
            </Label>
            <Input
              id="gs_receiver"
          ...
          <SearchableSelect
            ...
            placeholder={
              isLoadingDestinations
                ? 'Loading...'
                : `Select ${processingMode === ProcessingMode.TRANSFORM ? 'webhook' : 'partner'} destination`
            }
