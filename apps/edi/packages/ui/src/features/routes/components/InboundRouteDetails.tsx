...
import { useUpdateInboundRouteMutation } from '../api/inboundRouteHooks';
...
export function InboundRouteDetails({
  route,
  onCancel,
}: {
  route: InboundRouteItem;
  onCancel?: () => void;
}) {
  const { toast } = useToast();
  const updateRoute = useUpdateInboundRouteMutation();
  const isSubmitting = updateRoute.isPending;

  const { data: destinations } = useTenantDestinations('INBOUND');
  ...
  const {
    ...
    defaultValues: {
      ...
      processing_mode: route.processing_mode || 'TRANSFORM',
    },
  });

  const processingMode = watch('processing_mode');

  const onSubmit = (formData: InboundRouteFormData) => {
    const initialTargetId = route.webhook_id || route.as2_partner_id || route.sftp_partner_id || '';
    const payload: Partial<InboundRouteFormData> & {
    ...
    updateRoute.mutate(
      { routeId: route.route_id, payload },
      {
        ...
        onError: (err) => {
          toast({
            title: 'Error',
            description: err.message || 'Failed to update inbound route.',
            variant: 'destructive',
          });
        },
      },
  ...
  const targetDirty =
    targetId !== (route.webhook_id || route.as2_partner_id || route.sftp_partner_id || '');
