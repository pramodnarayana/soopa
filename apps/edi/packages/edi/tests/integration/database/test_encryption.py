...
def test_fernet_initialization_with_key(adapter, monkeypatch):
...
def test_fernet_initialization_without_key(adapter, monkeypatch):
...
def test_fernet_initialization_with_invalid_key(adapter, monkeypatch):
    monkeypatch.setenv("DB_ENCRYPTION_KEY", "invalid_key")
    with pytest.raises(RuntimeError):
        _ = adapter.fernet


def test_encrypt_decrypt_success(adapter, monkeypatch):
    key = Fernet.generate_key().decode("utf-8")
    monkeypatch.setenv("DB_ENCRYPTION_KEY", key)
...
def test_encrypt_without_key_raises_error(adapter, monkeypatch):
    monkeypatch.delenv("DB_ENCRYPTION_KEY", raising=False)
...
def test_decrypt_without_key_raises_error(adapter, monkeypatch):
    monkeypatch.delenv("DB_ENCRYPTION_KEY", raising=False)
    # Trigger initialization attempt
    _ = adapter.fernet
    with pytest.raises(RuntimeError, match="DB_ENCRYPTION_KEY not configured"):
...
def test_decrypt_with_invalid_token(adapter, monkeypatch):
    key = Fernet.generate_key().decode("utf-8")
    monkeypatch.setenv("DB_ENCRYPTION_KEY", key)
    # Trigger initialization
    assert adapter.fernet is not None
