...
def test_existing_sftp_connection_failures(client, fake_uow):
    ...
    with patch(
        "edi.application.use_cases.sftp_partners.create_sftp_partner_use_case.CreateSFTPPartnerUseCase.create_sftp_partner",
    ...
    with patch(
        "edi.application.use_cases.sftp_partners.update_sftp_partner_use_case.UpdateSFTPPartnerUseCase.update_sftp_partner",
        side_effect=ValueError("Bad value"),
    ):
        resp = client.put(
            f"/api/v1/tenants/1/edi/trading-partners/sftp/{p_id}",
