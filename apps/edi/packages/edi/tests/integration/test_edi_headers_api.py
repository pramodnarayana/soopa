...
async def test_edi_header_lifecycle(client: AsyncClient):
    ...
    res_create = await client.post("/api/v1/tenants/1/edi/edi-headers", json=payload)
    ...
    res_list = await client.get("/api/v1/tenants/1/edi/edi-headers")
    ...
    res_patch = await client.patch(
        f"/api/v1/tenants/1/edi/edi-headers/{header_id}", json=patch_payload
    )
    ...
    res_list_after = await client.get("/api/v1/tenants/1/edi/edi-headers")
    ...
    # 5. Delete EDI Header
    res_delete = await client.delete(f"/api/v1/tenants/1/edi/edi-headers/{header_id}")
    assert res_delete.status_code == 204, f"Failed to delete EDI header: {res_delete.text}"

    # 6. Verify deletion
    res_list_final = await client.get("/api/v1/tenants/1/edi/edi-headers")
...
async def test_edi_header_not_found(client: AsyncClient):
    ...
    res_delete = await client.delete(f"/api/v1/tenants/1/edi/edi-headers/{fake_id}")
