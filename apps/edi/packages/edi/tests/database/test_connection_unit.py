...
async def test_get_global_session(router: DatabaseRouter) -> None:
    ...
    with patch(
        "edi.adapters.outbound.database.connection.async_sessionmaker", return_value=mock_factory
    ):
...
async def test_close_all_disposes_engines(router: DatabaseRouter) -> None:
    ...
    mock_engine1 = typing.cast(AsyncMock, engine1)
    mock_global_engine = typing.cast(AsyncMock, global_engine)
    assert mock_engine1.dispose.call_count == 1
    assert mock_global_engine.dispose.call_count == 1
