...
def upgrade() -> None:
    ...
    sa.Column('tenant_id', sa.String(length=128), server_default=sa.text("current_setting('app.current_tenant')::varchar"), nullable=False),
    ...
    sa.CheckConstraint('(payload IS NOT NULL OR storage_uri IS NOT NULL)', name='chk_apigw_data_or_uri'),
    sa.PrimaryKeyConstraint('id')
    )
    ...
    sa.Column('created_at', sa.DateTime(timezone=True), nullable=False),
    sa.Column('updated_at', sa.DateTime(timezone=True), nullable=False),
    sa.CheckConstraint('(payload IS NOT NULL OR storage_uri IS NOT NULL)', name='chk_edi_json_data_or_uri'),
    sa.PrimaryKeyConstraint('id')
    )
    op.create_index('ix_edi_json_business_metadata', 'edi_json', ['business_metadata'], unique=False, postgresql_using='gin')
    op.create_index('ix_edi_json_sender_recv', 'edi_json', ['sender_id', 'receiver_id', 'created_at'], unique=False)
    ...
    op.create_index(op.f('ix_edi_json_trading_partner_id'), 'edi_json', ['trading_partner_id'], unique=False)
    op.create_index(op.f('ix_edi_json_transaction_type'), 'edi_json', ['transaction_type'], unique=False)
    ...
    sa.Column('created_at', sa.DateTime(timezone=True), nullable=False),
    sa.Column('updated_at', sa.DateTime(timezone=True), nullable=False),
    sa.CheckConstraint('(edi_data IS NOT NULL OR storage_uri IS NOT NULL)', name='chk_edi_msg_data_or_uri'),
    sa.PrimaryKeyConstraint('id')
    )
    ...
    op.create_index(op.f('ix_edi_messages_trading_partner_id'), 'edi_messages', ['trading_partner_id'], unique=False)
    ...
    op.create_index(op.f('ix_outbound_edi_headers_tenant_id'), 'outbound_edi_headers', ['tenant_id'], unique=False)
    op.create_index('ix_outbound_edi_headers_unique_trading_partner_id', 'outbound_edi_headers', ['tenant_id', 'trading_partner_id'], unique=True)
    ...
    op.create_index(op.f('ix_as2_partnerships_tenant_id'), 'as2_partnerships', ['tenant_id'], unique=False)
    ...
    op.create_index('ix_inbound_routes_unique_active', 'inbound_routes', ['tenant_id', 'isa_sender_id', 'isa_receiver_id', 'transaction_type'], unique=True, postgresql_where=sa.text('active = true'))
...
def downgrade() -> None:
    """Downgrade schema."""
    # ### commands auto generated by Alembic - please adjust! ###
    op.drop_index('ix_outbound_routes_unique_trading_partner_id', table_name='outbound_routes', postgresql_where=sa.text('active = true'))
    op.drop_index(op.f('ix_outbound_routes_tenant_id'), table_name='outbound_routes')
    op.drop_table('outbound_routes')
    op.drop_index('ix_inbound_routes_unique_active', table_name='inbound_routes', postgresql_where=sa.text('active = true'))
    ...
    op.drop_index('ix_outbound_edi_headers_unique_trading_partner_id', table_name='outbound_edi_headers')
